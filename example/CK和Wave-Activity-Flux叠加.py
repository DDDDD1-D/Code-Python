...
for myseason in ["SON","OND","NDJ"]:
	ss = xr.open_dataset("CK-%s.nc" % myseason)
	...
	levels = np.array([-2.0,-1.5,-1.0,-0.5,0,0.5,1.0,1.5,2.0])
	
	im = ax.contourf(lon1, lat, ck, levels=levels, cmap=cmps.BlueDarkRed18, transform=ccrs.PlateCarree(), extend="both")
	...
	fig.show()
	fig.savefig("waf-ck-%s.png" % myseason, dpi=1000)
