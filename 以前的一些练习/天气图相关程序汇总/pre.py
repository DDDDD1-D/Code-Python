...
files05=np.loadtxt("/Users/QQF/Downloads/obs/pre/08",dtype=np.str)
...
station05=[int(x) for x in files05[:,0] if x]
latitude05=np.array([float(x) for x in files05[:,1] if x]) 
longitude05=np.array([float(x) for x in files05[:,2] if x]) 
pre05=np.array([float(x) for x in files05[:,4] if x]) 
...
for ii in range(len(station01)):
    ...
    for jj in range(len(station04)):
        if station01[ii]==station04[jj]:
           tmp=tmp+pre04[jj]
    for jj in range(len(station05)):
        if station01[ii]==station05[jj]:
           tmp=tmp+pre05[jj]
           PRE.append(tmp)
