...
#
    ...
    var_reg_xr = var_reg_xr.loc[90:50,90:270]
    reg_sig_xr = reg_sig_xr.loc[90:50,90:270]
  ...
  # plot var
    ...
    im = ax.contourf(plon, plat, var_reg_xr, levels=levels, cmap=cmps.BlueYellowRed, transform=ccrs.PlateCarree(), extend="both")
  
    cb = plt.colorbar(im, orientation='horizontal', ticks=levels[::2], shrink=0.8)
    cb.ax.tick_params(labelsize=18)
  
  # plot significant regions
    sig1 = ax.contourf(plon, plat, reg_sig_xr, [np.min(reg_sig_xr),0.1], hatches=['..'], colors="None", zorder=1, transform=ccrs.PlateCarree())
